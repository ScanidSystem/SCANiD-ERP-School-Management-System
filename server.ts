...
async function startServer() {
  ...
  let teachers = [
    {
      ...
      user: {
        id: 3,
        fullName: "Primary Teacher 01",
        email: "teacher01@scanid.com",
      },
    },
  ];
  ...
  let subCastes = [
    { id: 1, casteId: 2, name: "General" },
    { id: 2, casteId: 2, name: "Kunbi" },
  ];
  let religions = [
    { id: 1, name: "HINDU" },
    { id: 2, name: "MUSLIM" },
  ...
  let notifications = [
    {
      id: 1,
      title: "System Update",
      message: "New academic module is live.",
      type: "info",
      isRead: false,
    ...
    },
    {
      id: 2,
      title: "Fee Reminder",
      message: "Late fee applies after 30th May.",
      type: "warning",
      isRead: true,
      createdAt: new Date().toISOString(),
    },
  ...
  let messages = [
    {
      ...
      content: "Let's discuss the new curriculum.",
      isRead: false,
      type: "Direct",
      createdAt: new Date().toISOString(),
    },
  ...
  let navigationItems = [
    ...
    },
    {
      ...
      title: "Role Master",
      icon: "Shield",
      path: "/configuration/role-master",
      parentId: 13,
      sortOrder: 1,
      roleIds: [1, 2],
    },
    {
      ...
      icon: "UserCheck",
      path: "/configuration/role-assignment",
      parentId: 13,
      sortOrder: 2,
      roleIds: [1, 2],
    },
    ...
    },
    {
      ...
      icon: "Calendar",
      path: "/configuration/academic-years",
      parentId: 18,
      sortOrder: 3,
      roleIds: [1, 2],
    },
    ...
    },
    {
      ...
      icon: "Droplets",
      path: "/configuration/blood-groups",
      parentId: 45,
      sortOrder: 2,
      roleIds: [1, 2],
    },
    ...
    },
    {
      ...
      icon: "UserCheck",
      path: "/configuration/admission-types",
      parentId: 45,
      sortOrder: 6,
      roleIds: [1, 2],
    },
  ...
  app.get("/api/database/schema", (req, res) =>
    res.json({
      data: { schema: "Mock In-Memory DB", tables: Object.keys(mastersMap) },
    }),
  );
  app.get("/api/database/script", (req, res) =>
    res.json({
      data: {
        content:
          "-- Mock SQL Server Schema --\nCREATE TABLE Students (\n  Id INT PRIMARY KEY IDENTITY,\n  GRNO NVARCHAR(50),\n  FullName NVARCHAR(200)\n);",
      },
    }),
  );
  app.get("/api/database/seed", (req, res) =>
    res.json({
      data: {
        content:
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (
      ...
      res.json({
        id: "1",
        name:
          username === "superadmin" ? "Global Admin" : "School Administrator",
        email:
          username === "superadmin" ? "admin@scanid.com" : "admin@scanid.com",
  ...
  app.get("/api/students", (req, res) => {
    ...
    const standardId = req.query.standardId
      ? parseInt(req.query.standardId as string)
      : null;
    const sectionId = req.query.sectionId
      ? parseInt(req.query.sectionId as string)
      : null;
    ...
    if (academicYearId) {
      filtered = filtered.filter(
        (s) =>
          ...
          s.academicyear === academicYearId.toString(),
      );
    }
    if (standardId) {
      filtered = filtered.filter(
    ...
    if (sectionId) {
      filtered = filtered.filter(
        (s) => s.sectionId === sectionId || s.SectionId === sectionId,
      );
    }
  ...
  app.post("/api/students", (req, res) => {
    ...
    const newStudent = {
      ...
      sms:
        body.sms === "true" || body.sms === true || body.Sms === true || false,
      isStateBoard:
        body.isStateBoard === "true" ||
        body.isStateBoard === true ||
        body.IsStateBoard === true ||
        false,
      digitalUniform:
        body.digitalUniform === "true" ||
        body.digitalUniform === true ||
        body.DigitalUniform === true ||
        false,
      digitalNotebook:
        body.digitalNotebook === "true" ||
        body.digitalNotebook === true ||
        body.DigitalNotebook === true ||
        false,
  ...
  app.post("/api/students/bulk", (req, res) => {
    ...
    const dbRegs = new Set<string>(
      students
        .map((s) =>
          (s.registrationNumber || s.grNo || s.grno || s.GRNO || "")
            .toString()
            .trim()
            .toLowerCase(),
        )
    ...
    const dbAadhars = new Set<string>(
      students
        .map((s) =>
          (s.aadharCard || s.aadharcard || "").toString().trim().toLowerCase(),
        )
        .filter(Boolean),
    );
    const dbRfids = new Set<string>(
      students
        .map((s) => (s.rfid || s.RFID || "").toString().trim().toLowerCase())
        .filter(Boolean),
    );
    const dbUniforms = new Set<string>(
      students
        .map((s) =>
          (s.uniformId || s.uniformid || "").toString().trim().toLowerCase(),
        )
        .filter(Boolean),
    );
    ...
    for (let idx = 0; idx < incoming.length; idx++) {
      ...
      const reg = (s.registrationNumber || s.grNo || s.grno || s.GRNO || "")
        .toString()
        .trim()
        .toLowerCase();
      if (reg) {
        if (batchRegs.has(reg) || dbRegs.has(reg)) {
          return res.status(400).json({
            message: `Row ${index}: Duplicate Registration Number/GRNO '${s.registrationNumber || s.grNo || s.grno || s.GRNO}' detected.`,
          });
        }
        batchRegs.add(reg);
      }
      ...
      // b) AadharCard
      const aadhar = (s.aadharCard || s.aadharcard || "")
        .toString()
        .trim()
        .toLowerCase();
      if (aadhar) {
        if (batchAadhars.has(aadhar) || dbAadhars.has(aadhar)) {
          return res.status(400).json({
            message: `Row ${index}: Duplicate Aadhar Card '${s.aadharCard || s.aadharcard}' detected.`,
          });
        }
      ...
      if (rfid) {
        if (batchRfids.has(rfid) || dbRfids.has(rfid)) {
          return res.status(400).json({
            message: `Row ${index}: Duplicate RFID/CardID '${s.rfid || s.RFID}' detected.`,
          });
        }
        batchRfids.add(rfid);
      }
      ...
      // d) UniformID
      const uniform = (s.uniformId || s.uniformid || "")
        .toString()
        .trim()
        .toLowerCase();
      if (uniform) {
        if (batchUniforms.has(uniform) || dbUniforms.has(uniform)) {
          return res.status(400).json({
            message: `Row ${index}: Duplicate UniformID '${s.uniformId || s.uniformid}' detected.`,
          });
        }
    ...
    const newItems = incoming.map((s: any, idx: number) => ({
      id:
        students.length > 0
          ? Math.max(...students.map((item) => item.id)) + idx + 1
          : idx + 1,
      ...s,
    }));
  ...
  app.put("/api/students/:id", (req, res) => {
    ...
    if (index !== -1) {
      ...
      const updated = {
        ...
        motherName:
          body.motherName || body.MOTHERNAME || students[index].motherName,
        fatherContactNo:
          body.fatherContactNo ||
          body.MOBILE ||
          students[index].fatherContactNo,
        motherContactNo:
          body.motherContactNo ||
          body.contact2 ||
          students[index].motherContactNo,
        sms:
          body.sms === "true" ||
          body.sms === true ||
          body.Sms === true ||
        ...
        isStateBoard:
          ...
          body.isStateBoard === true ||
          body.IsStateBoard === true ||
          students[index].isStateBoard,
        digitalUniform:
          body.digitalUniform === "true" ||
          body.digitalUniform === true ||
          body.DigitalUniform === true ||
          students[index].digitalUniform ||
          false,
        digitalNotebook:
          body.digitalNotebook === "true" ||
          body.digitalNotebook === true ||
          body.DigitalNotebook === true ||
          students[index].digitalNotebook ||
          false,
        fullName:
          body.fullName ||
          `${body.firstName || body.FNAME || students[index].firstName || ""} ${body.lastName || body.LNAME || students[index].lastName || ""}`.trim(),
      };
  ...
  app.post("/api/schools/:id/photo", (req, res) => {
    ...
    const mockPath =
      "https://api.dicebear.com/7.x/initials/svg?seed=" + id + "_" + Date.now();
    if (index !== -1) {
      // @ts-ignore
  ...
  app.get("/api/teachers", (req, res) => {
    ...
    if (subject) {
      filtered = filtered.filter(
        (t) => t.subject === subject || t.department === subject,
      );
    }
  ...
  app.get("/api/navigation", (req, res) => {
    ...
    if (roleId !== null) {
      ...
      if (roleId !== 1) {
        filtered = navigationItems.filter(
          (item) =>
            Array.isArray(item.roleIds) &&
            (item.roleIds.includes(roleId) || item.roleIds.includes(0)),
        );
      }
    }
    res.json({ data: filtered });
    ...
    const newItem = {
      id:
        navigationItems.length > 0
          ? Math.max(...navigationItems.map((n) => n.id)) + 1
          : 1,
      ...req.body,
  ...
  const apiProxy = createProxyMiddleware({
    ...
    pathFilter: (pathname, req) => {
      return (
        ...
        pathname.startsWith("/SCANiD_ERP_API")
      );
    },
    on: {
      error: (err, req, res) => {
        ...
        if (!req.url?.includes("health")) {
          console.warn(
            `[Proxy] Backend unreachable at http://127.0.0.1:5000: ${err.message}`,
          );
        }
        ...
        if ("writeHead" in res && !res.headersSent) {
          ...
          res.end(
            JSON.stringify({
              ...
              message:
                "The .NET backend is offline. Please start it for full functionality.",
            }),
          );
        }
      },
  ...
  httpServer
    .listen(PORT, "0.0.0.0", () => {
      ...
      console.log(
        `[INFO] Unified Server: Serving React Frontend and Express API`,
      );
      console.log(
        `[HINT] Use http://localhost:${PORT} to access the application.`,
      );
    })
    .on("error", (err: any) => {
      if (err.code === "EADDRINUSE") {
        ...
        console.error(
          `Please stop the application already running on port ${PORT} and try again.`,
        );
        console.error(
          `If you are a developer, ensure no other "npm run dev" or backend process is active on this port.\n`,
        );
        process.exit(1);
      } else {
